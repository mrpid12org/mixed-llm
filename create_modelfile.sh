#!/bin/bash
# SCRIPT V1.2 - Works in the /workspace/temp_gguf directory.
...
echo "--- Searching for GGUF in: $WORK_DIR ---"

# Find the largest .gguf file in the target directory.
GGUF_FILE=$(ls -S "$WORK_DIR"/*.gguf 2>/dev/null | head -n 1)

if [ -z "$GGUF_FILE" ]; then
    echo "--- ERROR: No .gguf files found in $WORK_DIR. ---"
    exit 1
fi

# Get just the filename, not the full path
GGUF_FILENAME=$(basename "$GGUF_FILE")
echo "--- Found GGUF file: $GGUF_FILENAME ---"

# Derive the Modelfile name from the GGUF file name
MODELFILE_NAME="${GGUF_FILENAME%.gguf}.Modelfile"
MODELFILE_PATH="${WORK_DIR}/${MODELFILE_NAME}"

echo "--- Creating Modelfile at: $MODELFILE_PATH ---"

# Create the Modelfile using a heredoc
cat <<EOF > "$MODELFILE_PATH"
# Set the GGUF file to use
# IMPORTANT: When running 'ollama create', you must be in the directory containing the GGUF file.
FROM ./$GGUF_FILENAME
...
echo "--- Modelfile '$MODELFILE_NAME' created successfully in '$WORK_DIR'. ---"
echo "--- Now, cd into '$WORK_DIR' and run: ---"
echo
echo "ollama create your-new-model-name -f \"$MODELFILE_PATH\""
echo
