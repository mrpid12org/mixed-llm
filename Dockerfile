...
# v9.3-slim-build-final
# Fixes the fsspec dependency issue in Text-Generation-WebUI
...
# --- FIX: Added 'fsspec' to the text-generation-webui requirements ---
RUN --mount=type=cache,target=/root/.cache/pip \
    ...
    /opt/venv-textgen/bin/python3 -m pip install --no-cache-dir exllamav2==0.0.15 ctransformers fsspec
