...
ARG OPENWEBUI_VERSION=v0.6.26
RUN apk add --no-cache curl
WORKDIR /app
RUN git clone --depth=1 --branch ${OPENWEBUI_VERSION} https://github.com/open-webui/open-webui.git .
RUN curl -L -o /app/CHANGELOG.md https://raw.githubusercontent.com/open-webui/open-webui/${OPENWEBUI_VERSION}/CHANGELOG.md
...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked --mount=type=cache,target=/var/lib/apt,sharing=locked \
    ...
    apt-get clean && rm -rf /var/lib/apt/lists/*
RUN --mount=type=cache,target=/root/.cache/pip pip3 install --no-cache-dir runpod
